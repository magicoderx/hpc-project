...
int main(){
    ...
    DATA_TYPE *d_data, *d_mean, *d_stddev, *d_symmat;

    // Memory allocation host side (pinned solution)
    cudaMallocHost((void**)&h_data,   m * n * sizeof(DATA_TYPE));
    cudaMallocHost((void**)&h_mean,   m * sizeof(DATA_TYPE));
    cudaMallocHost((void**)&h_stddev, m * sizeof(DATA_TYPE));
    cudaMallocHost((void**)&h_symmat, m * m * sizeof(DATA_TYPE));


    /* Initialize array(s). */
    init_array(m, n, &float_n, h_data);

    // Memory allocation device side
    ...
    // Copy data on device
    cudaMemcpy(d_data, h_data, m * n * sizeof(DATA_TYPE), cudaMemcpyHostToDevice);
    ...
    mean_kernel<<<num_blocks, blocksize>>>(d_data, d_mean, m, n, float_n);
    cudaCheckErr();

    /* 2. Determine standard deviations of column vectors of data matrix. */
    stddev_kernel<<<num_blocks, blocksize>>>(d_data, d_mean, d_stddev, m, n, float_n, 0.1f);
    cudaCheckErr();
    ...
    normalize_kernel<<<num_blocks2d, blocksize2d>>>(d_data, d_mean, d_stddev, m, n, float_n);
    cudaCheckErr();

    /* 4. Calculate the m * m correlation matrix. */
    correlation_kernel<<<num_blocks2d, blocksize2d>>>(d_data, d_symmat, m, n);
    cudaCheckErr();

    // Copy back results
    cudaMemcpy(h_symmat, d_symmat, m * m * sizeof(DATA_TYPE), cudaMemcpyDeviceToHost);
